...
namespace Alcazar.Playpen.Controllers.OData
{
	/// <summary>
	/// ODATa controller for Dategrid/Dates
	/// Challenged because it serialises dates to local time, which is not what we want.
	/// </summary>
	public class ODataDatesController : ODataController
	{
		private readonly ILogger<ODataDatesController> _logger;

		public ODataDatesController(ILogger<ODataDatesController> logger)
		{
			_logger = logger;
		}

		[EnableQuery]
		public IQueryable<DateItem> Get()
		{
			_logger.LogInformation("OData Dates Get called");

			try
			{
				// Generate sample data
				var dataList = new List<DateItem>();
				...
				_logger.LogInformation($"Generated {dataList.Count} date items for OData");
			...
			{
				_logger.LogError($"Error in OData Dates Get: {ex.Message}");
